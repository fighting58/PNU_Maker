...
### 4. Automation & System Utilities
- **`startup-script-builder`**: Generates robust, portable Windows startup scripts with graceful shutdown and privilege management.
...
3. **Chain**: If the task is multi-phase, outline the sequence (e.g., `brainstorming` -> `writing-plans` -> `frontend-design`).
